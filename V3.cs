...
internal class V3
{
    public static async Task PurgeFile(string cssOutput)
    {
    ...
    public static async Task ExtractCssClassesToOutputFile(string cssOutput, ProcessStartInfo processInfo)
    {
        ...
        using (var output = File.Open(Helpers.OutputPath!, FileMode.Create, FileAccess.Write))
        {
            await JsonSerializer.SerializeAsync(output, variants);
        }
        using (var spacingOutput = File.Open(Helpers.SpacingOutputPath!, FileMode.Create, FileAccess.Write))
        {
            await JsonSerializer.SerializeAsync(spacingOutput, spacing);
        }
        using (var opacityOutput = File.Open(Helpers.OpacityOutputPath!, FileMode.Create, FileAccess.Write))
        {
            await JsonSerializer.SerializeAsync(opacityOutput, opacities);
        }
        using var colorAndDarknessToRgbOutput = File.Open(Helpers.TailwindRgbOutputPath!, FileMode.Create, FileAccess.Write);
        await JsonSerializer.SerializeAsync(colorAndDarknessToRgbOutput, colorAndDarknessToRgb);

        await GetCssDescripts(cssOutput, colors, spacing, variants.Where(v => v.UseSpacing == true).ToList());

        await GetSortOrder(processInfo, variants, colors[0], spacing.Last());
        await GetSortOrderModifiers(processInfo);
    }

    private static async Task GetCssDescripts(string cssOutput, List<string> colors, List<string> spacings, List<Variant> variants)
    {
        Console.WriteLine();
        ...
        using var cssDescOutput = File.Open(Helpers.CssOutputPath!, FileMode.Create, FileAccess.Write);
        await JsonSerializer.SerializeAsync(cssDescOutput, dict);
    }

    private static async Task GetSortOrderModifiers(ProcessStartInfo processInfo)
    {
        List<string> modifiers;

        using (var file = File.Open(Helpers.ModifiersPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
        {
            modifiers = (await JsonSerializer.DeserializeAsync<List<string>>(file))!;
        }

        using (var file = File.Open(Helpers.TestHtmlPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
        {
        ...
        using (var file = File.Open(Helpers.TestHtmlPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
        {
        ...
        using (var file = File.Open(Helpers.ModifiersOrderOutputPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
        {
    ...
    private static async Task GetSortOrder(ProcessStartInfo processInfo, List<Variant> variants, string firstColor, string lastSpacing)
    {
        ...
        using (var file = File.Open(Helpers.OrderOutputPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
        {
            await JsonSerializer.SerializeAsync(file, sorted);
        }
